...
std::shared_ptr<Texture2D> Renderer::s_WhiteTexture;
...
void Renderer::Init() {
    ...
    s_WhiteTexture = Texture2DCreate(1, 1, &data);
}
...
void Renderer::Submit(const std::shared_ptr<VertexArray>& vertexArray, glm::mat4& transform) {
    vertexArray->Bind();
    const auto& shader = s_ShaderLibrary->Get(s_SceneData->SceneShader);
    shader->Bind();
